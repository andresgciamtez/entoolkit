"""
Epanettk wrapper from EPANET Programmers Toolkit
"""
...
if os.name in ['nt','dos']:
    if '64' in platform.machine():
        libf = 'epanet/Windows/%s.dll'%('epanet2_amd64')
    else:
        libf = 'epanet/Windows/%s.dll'%('epanet2')    
    
elif sys.platform in ['darwin']:
    libf = 'epanet/Darwin/lib%s.dylib'%('epanet2')
else:
    libf = 'epanet/Linux/lib%s.so'%('epanet')

libepanet = resource_filename('epanettk', libf)
_lib = ctypes.windll.LoadLibrary(libepanet)
